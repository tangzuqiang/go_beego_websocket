	...
	manager.Clients.Range(func(k, v interface{}) bool {
		...
		if conn != ignore {
			...
			if popType == "browser" || popType == "all" {
				if !utils2.InSliceIface(conn.UserId, sendUserIds) {
...
func (manager *ClientManager) SendMsgToUsers(message []byte, userIds []interface{}) []interface{} {
	...
	sendUserIds = utils2.SliceUnique(sendUserIds)

	return sendUserIds
}

//更新消息弹窗类型
func (manager *ClientManager) UpdateMsgPopType(popType string, options data2.MessageOptions, msg data2.ResMessage) []byte {
...
//获取消息弹窗类型
func (manager *ClientManager) GetMsgPopType(message []byte) (string, data2.MessageOptions, data2.ResMessage) {
	var msg data2.ResMessage
	json.Unmarshal(message, &msg)
	//获取消息弹窗类型默认all
	popType := "all"
	var options data2.MessageOptions
	if msg.Data.Options != "" {
	...
	go func() {
		for range ticker.C {
			manager.Clients.Range(func(k, v interface{}) bool {
				conn := k.(*Client)
				//超过时间关闭连接
				if time.Now().Unix()-conn.RegisterTime > data2.CLIENT_TIMEOUT {
					conn.Close()
				}
